...
namespace App\Http\Controllers\Frontend\User\Profile\MyRecipes;

use App\Enums\MyRecipesFilter;
...
/**
 * Class View
 * @package App\Http\Controllers\Frontend\User\Profile\MyRecipes
 */
class View extends AbstractBaseSearch
{
    ...
    protected function init()
    {
        ...
        $this->parametersKeys = MyRecipesFilter::labels();

        $this->singleParameters = [
            MyRecipesFilter::P_PAGESIZE
        ];
    }
    ...
    protected function render($data)
    {
        ...
        return view('user.profile.my_recipes', $data);
    }
    ...
    {
        /** @var Builder $query */
        $query = Recipe::select(['recipes.*'])->where('user_id', $this->currentUser->id);
    ...
    {
        $res = $this->getSelected(MyRecipesFilter::P_PAGESIZE)->first();
        ...
        $links = collect(config('kosher.pagination.per_page_selector'))
            ->mapWithKeys(function ($item) use ($currentLink, $vars, $current) {
                $vars[MyRecipesFilter::P_PAGESIZE] = $item;
                return [
                    [
        ...
        return [
            'key' => MyRecipesFilter::P_PAGESIZE,
            'selected' => $current,
            'all' => $links,
