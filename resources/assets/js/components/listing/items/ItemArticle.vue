<template>
    <li class="item-article">
        <a :href="item.url" class="item-article__visual">
            <img :src="item.searchListImage" :alt="item.title">
            <span class="item-recipe__video" v-if="item.hasVideo">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 86.3 86.3" enable-background="new 0 0 86.3 86.3" xml:space="preserve" class="icon icon-play">
        ...
        <div class="item-article__info">
            <a :href="item.category.url" class="item-article__category uppercase">
                {{ item.category.title }}
            </a>
            ...
            <div class="item-article__meta">
                <span class="date">{{ item.published }}</span>
                <span class="divider">|</span>
                <a :href="item.user.publicProfileUrl" :title="trans.tooltip.public_profile" class="user">{{ item.user.fullName }}</a>
            </div>
        </div>
    </li>
</template>
