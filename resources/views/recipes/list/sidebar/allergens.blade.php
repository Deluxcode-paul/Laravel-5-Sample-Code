<h3>@lang('recipes/list.allergens-title')</h3>
    ...
    @foreach($preferences['all'] as $preferenceId=>$preferencelabel)
            <div class="preference-block preference-block-{{$preferenceId}}  @if ($activePreference == $preferenceId) active  @else hidden @endif ">
                <div class="short-info">
                    @foreach($allergens[$preferenceId] as $id=>$label)
                        <input type="checkbox"
                               data-id="{{ $id }}"
                               data-counter="{{ (isset($i))?$i++:($i = 0) }}"
                               name="{{ $allergens['key'] }}[{{ $id }}]"
                               {{ $allergens['selected']->contains($label) ? 'checked="checked"' : '' }}>
                            {{ $label }}
                        </input><br/>
                        @if ($i==1)
                            </div>
